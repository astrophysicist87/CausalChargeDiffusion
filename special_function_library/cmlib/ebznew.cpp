...
namespace SFL
{
	...
	*/
	...
	double sum,term,tol=1.e-6,denom;
	...
	{
	double sum,term,denom,power,tol=1.e-6;
	int i;
	sum=0.;
	power=1.;
	for(i=0;i<itmax;i++)
	...
	{
	/* Catalan beta= sum k=0 to inf. (-1)^k (2k+1)^-s*/
	if(s==1.) return pi*.25;
	...
	{
	/* sum k=0 to infinity (2k+1)^-s */
	return (1.-pow(2.,-s))*zeta(s);
	}

	double eta(double s)
	{
	/* sum from k=1 to infinity -1^k k^-n */
