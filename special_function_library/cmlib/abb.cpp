...
namespace SFL
{
	...
	{
		double sum,sum2,term2,term,x,y,q,arg,c,s;
	...
	double kbes(double z, double nuu)
	{
		int n;
	...
	*/
	...
	{
		int i,itmax=40;
		double x,zeta,zi,arg,c,d,term,term2,old,old2,tol=1.e-6,factor,sum,sum2;
		if(z==0.) return c1a;
		x=abs(z);
		if(x>=asympt)
			{
	...
	{
		int i,itmax=40;
		double x,zeta,zi,arg,c,d,term,term2,old,old2,zz,tol=1.e-6,factor,sum,sum2;
	...
	{
		double x,y,zz,sum,term,mu,ser,asym;
		int k,n,itmax=500;
		x=z*.5;
		y=1.;
		if(nu!=0.)y= pow(x,nu);
	...
	{
		int k,itmax=40;
		double zeta,zi,arg,c,d,term,term2,old,old2,tol=1.e-6,factor,sum,sum2;
		double z,zz;
		if(x==0.)return -c2a;
		z=abs(x);
		if(z>=asympt)
