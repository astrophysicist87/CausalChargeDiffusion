...
namespace SFL
{
	...
	int Cpow(struct complex *base,struct complex *expon,struct complex *ans)
	{struct complex l,d;
	...
	double oldterm,newterm;int m,i;struct complex s,z;
	...
	for(i=0,term.x=sum.x=1.,term.y=sum.y=0.;i<top;i++)
	{
		...
		CADD(sum,sum,term);
		/*CDIV(ratio,term,sum);*/
		//printf("oldterm = %0.10e, newterm = %0.10e, term = (%0.10e, %0.10e), sum = (%0.10e, %0.10e)\n", oldterm, newterm, term.x, term.y, sum.x, sum.y);
		//printf("oldterm = %0.10e, newterm = %0.10e, |term| = %0.10e, tol*|sum| = %0.10e\n", oldterm, newterm, cabs(term), tol*cabs(sum));
	...
	cexp(x,&p);
	//printf( "cexp( (%0.15e, %0.15e ) ) = (%0.15e, %0.15e)\n", x->x, x->y, p.x, p.y );
	CMULT(sum,p,q);
	cgamma(a,&p,&q);
	//printf( "cgamma( (%0.15e, %0.15e ) ) = (%0.15e, %0.15e)\n", a->x, a->y, p.x, p.y );
	CDIV(term1,sum,p);
	CTREAL(z,z,-1.);
	...
	cpow(x,a,&p);
	//printf( "cpow( (%0.15e, %0.15e ), (%0.15e, %0.15e ) ) = (%0.15e, %0.15e)\n", x->x, x->y, a->x, a->y, p.x, p.y );
	CDIV(q,sum,p);CLET(r,*a);CTREAL(r,r,-pi);CTI(s,r);
	cexp(&s,&p);
	//printf( "cexp( (%0.15e, %0.15e ) ) = (%0.15e, %0.15e)\n", s.x, s.y, p.x, p.y );
	CMULT(sum,p,q);
	CSUB(r,*c,*a);cgamma(&r,&p,&q);
	CDIV(term2,sum,p);
